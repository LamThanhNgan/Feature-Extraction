...
import ContentFeatures_new as ctnfe_new
import HostFeatures as htfe
import LexicalFeatures as lxfe
...
PAGE_HTML = ""
FOLDER_RESULT_NAME = ""
HEADER = ['url','host', 'tld', 'scheme', 'url_length', 'path_length', 'host_length', 'host_is_ip', 'has_port_in_string', 'num_digits', 'parameters', 'fragments', 'is_encoded', 'string_entropy', 'alexa_dis_similarity', 'subdirectories', 'periods', 'has_client', 'has_login', 'has_admin', 'has_server', 'num_encoded_chars', 'page_entropy', 'num_script_tags', 'script_to_body_ratio', 'html_length', 'page_tokens', 'num_sentences', 'num_punctuations', 'distinct_tokens', 'capitalizations', 'avg_tokens_per_sentence', 'num_html_tags', 'num_hidden_tags', 'num_iframes', 'num_embeds', 'num_objects', 'hyperlinks', 'num_whitespaces', 'num_included_elemets', 'num_double_documents', 'num_suspicious_elements', 'num_eval_functions', 'avg_script_length', 'avg_script_entropy', 'num_suspicious_functions', 'num_subdomains', 'registration_date', 'expiration_date', 'last_updates_dates', 'age', 'intended_life_span', 'life_remaining', 'registrar', 'reg_country', 'host_country', 'open_ports', 'num_open_ports', 'is_live', 'isp', 'connection_speed', 'avg_update_days', 'total_updates', 'ttl', 'length_url', 'length_hostname', 'ip', 'nb_dots', 'nb_hyphens', 'nb_at', 'nb_qm', 'nb_and', 'nb_or', 'nb_eq', 'nb_underscore', 'nb_tilde', 'nb_percent', 'nb_slash', 'nb_star', 'nb_colon', 'nb_comma', 'nb_semicolumn', 'nb_dollar', 'nb_space', 'nb_www', 'nb_com', 'nb_dslash', 'http_in_path', 'https_token', 'ratio_digits_url', 'ratio_digits_host', 'punycode', 'port', 'tld_in_path', 'tld_in_subdomain', 'abnormal_subdomain', 'nb_subdomains', 'prefix_suffix', 'shortening_service', 'path_extension', 'nb_redirection', 'nb_external_redirection', 'length_words_raw', 'char_repeat', 'shortest_words_raw', 'shortest_word_host', 'shortest_word_path', 'longest_words_raw', 'longest_word_host', 'longest_word_path', 'avg_words_raw', 'avg_word_host', 'avg_word_path', 'phish_hints', 'domain_in_brand', 'brand_in_subdomain', 'brand_in_path', 'suspecious_tld', 'statistical_report', 'login_form', 'external_favicon', 'links_in_tags', 'submit_email', 'ratio_intMedia', 'ratio_extMedia', 'sfh', 'iframe', 'popup_window', 'safe_anchor', 'onmouseover', 'right_clic', 'empty_title', 'domain_in_title', 'domain_with_copyright', 'whois_registered_domain', 'domain_registration_length', 'dns_record', 'page_rank', 'tag']
...
def extract_features( ):
    ...
    try: 
        row = {
            'length_url': urlfe.url_length(SUBMITED_URL),
            ...
            'nb_at': urlfe.count_at(SUBMITED_URL),
            'nb_qm': urlfe.count_exclamation(SUBMITED_URL),
            'nb_and': urlfe.count_and(SUBMITED_URL),
            'nb_or': urlfe.count_or(SUBMITED_URL),
            'nb_eq': urlfe.count_equal(SUBMITED_URL),
            'nb_underscore': urlfe.count_underscore(SUBMITED_URL),
            'nb_tilde':  urlfe.count_tilde(SUBMITED_URL),
            'nb_percent': urlfe.count_percentage(SUBMITED_URL),
            'nb_slash': urlfe.count_slash(SUBMITED_URL),
            'nb_star': urlfe.count_star(SUBMITED_URL),
            'nb_colon': urlfe.count_colon(SUBMITED_URL),
            'nb_comma': urlfe.count_comma(SUBMITED_URL),
            'nb_semicolumn': urlfe.count_semicolumn(SUBMITED_URL),
            'nb_dollar': urlfe.count_dollar(SUBMITED_URL),
            'nb_space': urlfe.count_space(SUBMITED_URL),
            'nb_www': urlfe.check_www(words_raw),
            'nb_com': urlfe.check_com(words_raw),
            'nb_dslash': urlfe.count_double_slash(SUBMITED_URL),
            'http_in_path': urlfe.count_http_token(path),
            'https_token': urlfe.https_token(scheme),
            'ratio_digits_url': urlfe.ratio_digits(SUBMITED_URL),
            'ratio_digits_host': urlfe.ratio_digits(hostname),
            'punycode': urlfe.punycode(SUBMITED_URL),
            'port': urlfe.port(SUBMITED_URL),
            'tld_in_path': urlfe.tld_in_path(tld, path),
            'tld_in_subdomain': urlfe.tld_in_subdomain(tld, subdomain),
            'abnormal_subdomain': urlfe.abnormal_subdomain(SUBMITED_URL),
            'nb_subdomains': urlfe.count_subdomain(SUBMITED_URL),
            'prefix_suffix': urlfe.prefix_suffix(SUBMITED_URL),
            ...
            'phish_hints': urlfe.phish_hints(SUBMITED_URL),  
...
def Extracter(urlSubmiteds):
    ...
    nb = 0
    i = nb
    for urlSubmited in urlSubmiteds:
...
if __name__ == '__main__':
    db = pd.read_csv('data/data1-1000/data0.csv', on_bad_lines='skip')
    data = list(db['url'])
    data = list(set(data))
    try:
